...
class ToyModel(nn.Module):
    ...
    def forward(self, x):
        print("dtype of model params fc1:", self.fc1.weight.dtype)
        print("dtype of model params fc2:", self.fc2.weight.dtype)
        print("dtype of model params ln:", self.ln.weight.dtype)
        x = self.fc1(x)
        print("dtype of x after fc1:", x.dtype)
        x = self.ln(x)
        print("dtype of x after ln:", x.dtype)
        x = self.relu(x)
        x = self.fc2(x)
